...
from lib.training import DT
from lib.data import Data
from lib.data import fold_1simple, fold_10euqal, fold_3groups, fold_n_files
from lib.data import name_list_n_files

from lib.pipelines import TarDC
from lib.pipelines.Unpackers import TargetDataUnpacker as TDU


def dummy_test(run_name, dataset_name, create_model, unpack, pack, create_folding, name_list):
    """
    Dummy test.

    This model just return y_true + noise, as predict:
...
def main():
    name_list = [name_list_n_files] # correct names of foldings for plots
    dataset_name = 'real'

    for i, fold_method in enumerate([fold_n_files]):# loop for folding metheds
            for unpack_i in [TDU]: # loop unpacking types
                dummy_test(
                    f'DummyTest_{dataset_name}_{unpack_i.__name__[:-12]}_{fold_method.__name__[5:]}',
