...
from lib.data import fold_1simple
from models.model_configurations import oneway_model

from lib.pipelines import TarDC
from lib.pipelines.Unpackers import TargetDataUnpacker as TarDU


def simple_test(run_name, dataset_name, create_model, unpack, pack, create_folding, name_list):
    """
    Simple test.
...
def main():
    ...
    for i, fold_method in enumerate([fold_1simple]):# loop for folding metheds
            for unpack_i in [TarDU]: # loop unpacking types
                simple_test(
